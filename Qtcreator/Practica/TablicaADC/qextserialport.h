...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Изменил в POSIX-реализации функцию qint64 QextSerialPort::readData(char * data, qint64 maxSize)
/*Обнаружил, что она ведет себя при чтении данных из порта не совсем ожидаемо: функция read() разблокируется не только по
 * прошествии таймаута, но и по приему одного байта. Я сделал, чтобы она разблокировалась либо по приему всех запрошенных байтов,
 * либо по прошествии таймаута МЕЖДУ БАЙТАМИ. По логике надо бы сделать общий таймаут на всю запрошенную в read пачку байтов, но это
 * посложнее и приведет к необходимости динамической настройки таймаутов (во время приема байтов) и может привести к глюкам.
 *
 * */
...
class QEXTSERIALPORT_EXPORT QextSerialPort: public QIODevice
{
    ...
    public:
        ...
        QextSerialPort(QueryMode mode = Polling);
        QextSerialPort(const QString & name, QueryMode mode = Polling);
        QextSerialPort(PortSettings const& s, QueryMode mode = Polling);
        QextSerialPort(const QString & name, PortSettings const& s, QueryMode mode = Polling);
        ~QextSerialPort();
